...
{	
	unsigned int i,j;
	unsigned short tempI16s[32];
	
	//MLX90640_I2CRead(0x2400, 832, eeprom);
	for (i=0;i<26;i++)
	{
		MLX90640_I2CRead(0x2400+i*32, 32, tempI16s);
		for (j=0;j<32;j++)
			eeprom[i*32+j]=tempI16s[j];
		delay_us(10);
	}
}
void GetRAM(UINT16 *ram)
{
	...
	//MLX90640_I2CRead(0x0400, 832, ram);
	for (i=0;i<26;i++)
	{
		MLX90640_I2CRead(0x0400+i*32, 32, tempI16s);
		for (j=0;j<32;j++)
			ram[i*32+j]=tempI16s[j];
		delay_us(10);
	}
}
...
{
	UINT16 staReg;
	MLX90640_I2CRead(0x8000, 1, &staReg);//?MLX90640??????????
...
{
	UINT16 ctrReg;
	MLX90640_I2CRead(0x800D, 1, &ctrReg);//?MLX90640??????????

	return ctrReg;
}

SINT8 MLX90640_DataReady(void)//RAM??????,??-1?????,0??Frame0??,1??Frame1??
{
	UINT16 statusReg[1];

	MLX90640_I2CRead(0x8000, 1, statusReg);
	if (statusReg[0]&0x0008)//??????Frame
	{
